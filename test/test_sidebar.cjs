            ...
            call('Runtime.enable', {}).then(() => {
                return call('Runtime.evaluate', {
                    expression: `(function() {
                        ...
                        const sidebar = sidebars[0]; // Assuming it's the first one based on previous query
                        if (!sidebar) return null;
                        
                        // Let's get a summary of its structure
                        return sidebar.outerHTML.substring(0, 1000);
                    })()`,
                    returnByValue: true
                });
            }).then(res => {
