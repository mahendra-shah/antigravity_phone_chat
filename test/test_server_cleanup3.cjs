            ...
            call('Runtime.enable', {}).then(() => {
                return call('Runtime.evaluate', {
                    expression: `(function() {
                        ...
                        let matches = clone.innerHTML.match(/antigravity\\.agentSidePanelInputBox/g);
                        
                        let remainingInputs = Array.from(clone.querySelectorAll('[contenteditable="true"]')).map(el => el.id || el.className || 'unknown');
                        
                        return { matches: matches || [], remainingInputs };
                    })()`,
                    returnByValue: true
                });
            }).then(res => {
